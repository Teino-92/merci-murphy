...
export function bookingRescheduledHtml(params: {
  ...
  return emailHtml({
    ...
    body: [
      p('Bonjour,'),
      p(
        `Nous vous informons que ${subject} pour un <strong>${serviceName.toLowerCase()}</strong> a été déplacé.`
      ),
      p(`Votre nouveau rendez-vous est fixé au <strong>${newDate}</strong> chez merci murphy.`),
